  ...
  describe('insertDeviceDefinition', () => {
    ...
    context('Error handling', () => {
      it('Should revert if Device Definition table does not exist', async () => {
        await expect(
          ddTableInstance.connect(manufacturer1).insertDeviceDefinition(99, C.mockDdModel1, C.mockDdYear1)
        ).to.be.revertedWithCustomError(
          ddTableInstance,
          'TableDoesNotExist',
        ).withArgs(99);
      ...
      it('Should revert if caller is not the manufacturer ID owner or has the INSERT_DEVICE_DEFINITION_ROLE', async () => {
        await expect(
          ddTableInstance.connect(unauthorized).insertDeviceDefinition(1, C.mockDdModel1, C.mockDdYear1)
        ).to.be.revertedWithCustomError(
          ddTableInstance,
          'Unauthorized'
