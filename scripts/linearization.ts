...
task('print-linearization')
  .addPositionalParam(
    'contract',
    'The fully qualified contract name (contracts/File.sol:ContractName)'
  )
  .addFlag('noCompile', 'Skip compilation')
  .setAction(
    async (args: { contract: string, noCompile: boolean }, hre, runSuper) => {
      if (!args.noCompile) {
      ...
      for (const c of findAll(
        'ContractDefinition',
        buildInfo.output.sources[sourceName]?.ast
      )) {
