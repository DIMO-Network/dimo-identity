...
contract DIMORegistry is IDIMORegistry {
    ...
    function newRecord(bytes32 node, address _owner) external override {
    ...
    function setRecord(bytes32 node, address _owner) external override {
    ...
    /// @param _owner The address of the new owner
    function setSubnodeRecord(bytes32 node, bytes32 label, address _owner) external override authorized(node) {
    ...
    /// @param _owner The address of the new owner
    function setOwner(bytes32 node, address _owner) public override authorized(node) {
        _setOwner(node, _owner);
        emit Transfer(node, _owner);
    }
    ...
    /// @param label The hash of the label specifying the subnode
    /// @param _owner The address of the new owner
    function setSubnodeOwner(bytes32 node, bytes32 label, address _owner) public override authorized(node) returns(bytes32) {
